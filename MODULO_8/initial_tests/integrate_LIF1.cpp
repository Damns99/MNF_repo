...
// dV(n)/dt = (V(n+1) - V(n)) / h
std::vector<double> fwdEuler1(double V0, double h, int N, std::vector<double>& I, double params[6]) {
	double Cm = params[0], g = params[1], Vrest = params[2], Vth = params[3], Vreset = params[4], tauref = params[5];
	std::vector<double> V(N+1);
	int refr = 0;
	V[0] = V0;
	for(int n = 0; n < N; n++) {
		if(refr <= 0) V[n+1] = V[n] * (1 - g/Cm*h) + h/Cm * (I[n] + g * Vrest);
		else {
			V[n+1] = V[n] * (1 - g/Cm*h) + h/Cm * (g * Vrest);
			refr--;
		}
		if(V[n+1] >= Vth) {
			V[n+1] = Vreset;
...
std::pair<std::vector<double>,std::vector<double>> fwdEuler2(double V0, double t0, double h, int N, std::vector<double>& I, double params[6]) {
	double Cm = params[0], g = params[1], Vrest = params[2], Vth = params[3], Vreset = params[4], tauref = params[5];
...
std::vector<double> fwdEuler3(double V0, double h, int N, std::vector<double>& I, double params[6]) {
	double Cm = params[0], g = params[1], Vrest = params[2], Vth = params[3], Vreset = params[4], tauref = params[5];
	std::vector<double> V;
	int refr = 0;
	V.push_back(V0);
	for(int n = 0; n < N; n++) {
		if(refr <= 0) V.push_back(V[n] * (1 - g/Cm*h) + h/Cm * (I[n] + g * Vrest));
		else {
			V.push_back(V[n] * (1 - g/Cm*h) + h/Cm * (g * Vrest));
			refr--;
		}
		if(V[n+1] >= Vth) {
			V[n+1] = Vreset;
...
void fwdEuler4(double V0, double h, int N, double* I, double params[6], double* V) {
	double Cm = params[0], g = params[1], Vrest = params[2], Vth = params[3], Vreset = params[4], tauref = params[5];
	int refr = 0;
	V[0] = V0;
	for(int n = 0; n < N; n++) {
...
void fwdEuler5(double V0, double t0, double h, int N, double* I, double params[6], double* V, double* t) {
	double Cm = params[0], g = params[1], Vrest = params[2], Vth = params[3], Vreset = params[4], tauref = params[5];
	int refr = 0;
	V[0] = V0; t[0] = t0;
	for(int n = 0; n < N; n++) {
...
int main() {
	...
	std::vector<double> I = square_wave(N, 10000, deltaI/2, -10, deltaI/2); // [mA]
	double t0 = 0; // [ms]
	double V0 = -70; // [mV]
