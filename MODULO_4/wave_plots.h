...
namespace waveplots {
	...
	void plot(const std::vector<DblBcv> u, double t0, double dt, int nt, double x0, double dx, int nx, const std::string file, int mode, double mn, double mx) {
		gErrorIgnoreLevel = kWarning;
		...
		for(int j = 0; j < nx; j++) {
			tmpx[j] = x0 + j * dx;
			first[j] = bound(u[0][j], mn, mx);
			last[j] = bound(u[nt - 1][j], mn, mx);
		}
	...
	void plotFFT(const std::vector<std::vector<DblBcv>> u, double t0, double dt, int nt, const std::string file, int mode) {
		...
		for(int k = 0; k < u[0].size(); k++) {
			...
			for(int i = 0; i < nt; i++) {
				...
				for(int j = 1; j < nx / 2 + 1; j++) hist->Fill(t0 + i * dt, x0 + j * dx, real[j] * real[j] + imag[j] * imag[j]);
			}
