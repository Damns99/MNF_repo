...
void calculateEnergyMagnetization() {
	...
	for (int i = 0; i < length; i++) for (int j = 0; j < length; j++) {
		...
		int neighbours = spin[index2d(i-1,j,length)] + spin[index2d(i,j-1,length)] + spin[index2d_5th_link(i,j,length)];
...
int updateMetropolis() {
	...
	int neighbours = spin[index2d(i-1,j,length)] + spin[index2d(i+1,j,length)] + spin[index2d(i,j-1,length)] + spin[index2d(i,j+1,length)] + spin[index2d_5th_link(i,j,length)] + spin[index2d_6th_link(i,j,length)];
