...
int lnkd_spin(int i, int j, int L, int link) {
	...
	switch(type) {
		...
		case 1:
			switch(link) {
				case 0:
					if(node_type(i+1, j) == 0) return 0;
					return spin[index2d(i+1, j, L)];
				case 1:
					if(node_type(i, j+1) == 0) return 0;
				...
				case 3:
					...
					return spin[index2d(i, j-1, L)];
				case 4:
					if(node_type(i - 1, j - 1 + 2 * (i % 2)) == 0) return 0;
					return spin[index2d(i - 1, j - 1 + 2 * (i % 2), L)];
				case 5:
					if(node_type(i + 1, j - 1 + 2 * (i % 2)) == 0) return 0;
					return spin[index2d(i + 1, j - 1 + 2 * (i % 2), L)];
				default:
...
void createLattice(int len, double inv_temp, double extf, long seed, int init_mode, const std::string init_file = "") {
	length = len;
	assert(length > 0 && length * length <= MAX_LENGTH && (length % 14) == 0);
...
void calculateEnergyMagnetization() {
	...
	for (int i = 0; i < length; i++) for (int j = 0; j < length; j++) {
		...
		int neighbours = 0;
		for (int k = 0; k < 6; k++) neighbours += lnkd_spin(i, j, length, k);
		energy += - curr_spin * neighbours / 2.;
		magnetization += curr_spin;
	}
	
	double normalization = 1. / (length * length) * (49. / 36.);
