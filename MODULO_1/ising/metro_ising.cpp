...
int main(int argc, char* argv[]) {
    ...
    long seed;
    int nmeas, ncycles, length, nlinks, geom, init_mode, append, do_snaphots, do_cuda;
    ...
    parser.addOptParameter<int>("cuda", &do_cuda, 0, "[int] If != 0 parallelizes.");
	...
	for(int ii = 0; ii < nmeas; ii++) {
		...
		int acc = 0;
		for(int jj = 0; jj < ncycles; jj++) {
			if(do_cuda) cudaSquareUpdateMetropolis(&lattice);
			else for(int kk = 0; kk < length * length; kk ++) acc += lattice.updateMetropolis();
		}
