...
double distsq(int i, int j) {
	int xi = i % length, yi = i / length;
	int xj = j % length, yj = j / length;
	int si = ((xi + yi) % 2 == 0) ? 1 : -1;
	int sj = ((xj + yj) % 2 == 0) ? 1 : -1;
	double xi_ = xi - si*1./4.;
	double xj_ = xj - sj*1./4.;
	double dx = abs(xi_ - xj_);
	double dy = abs(yi - yj);
	dx = min(dx, length - dx);
	dy = min(dy, length - dy) * sqrt(3.) / 2.;
	return dx * dx + dy * dy; 
}
