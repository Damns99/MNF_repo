...
int main(int argc, char* argv[]) {
    std::string folder = "measures", tmp_folder, measfilename, outname;
	...
	e_canvas->cd();
	e_canvas->SetGrid();
	e_graph->SetTitle((tmp_folder + ";time;#varepsilon").c_str());
	e_graph->Draw("A*");
	...
	m_canvas->cd();
	m_canvas->SetGrid();
	m_graph->SetTitle((tmp_folder + ";time;M").c_str());
	m_graph->Draw("A*");
	...
	eh_canvas->cd();
	eh_canvas->SetGrid();
	e_hist->SetTitle((tmp_folder + ";#varepsilon;").c_str());
	e_hist->Draw("HIST");
	...
	mh_canvas->cd();
	mh_canvas->SetGrid();
	m_hist->SetTitle((tmp_folder + ";M;").c_str());
	m_hist->Draw("HIST");
